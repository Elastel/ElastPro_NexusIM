...
{
    $name = "nexus.png";
    echo '<img src="app/img/'.$name.'" class="navbar-logo" class="img-fluid" style="max-width: 3rem;">';
}

function setLoginGuide($target, $hostname)
{
    echo '<a href="https://nexusim.com/" class="mt-2 d-block text-decoration-none" target="_blank">
        ...
        if ($dev_model == 'EG324Pro') {
            ...
            foreach ($cpuinfo_array as $line) {
                if (strpos($line, 'Revision') === 0) {
                    ...
                    if (isset($parts[1])) {
                        $rev = trim($parts[1]);
                        $rev = str_replace("Raspberry Pi", "NexusIM", $rev);
                    }
                    break;
                }
            }
            ...
            if (array_key_exists($rev, $revisions)) {
                return str_replace("Raspberry Pi", "NexusIM", $revisions[$rev]);
            } else {
                $model = trim(file_get_contents('/proc/device-tree/model'));
                if (isset($model)) {
                    return str_replace("Raspberry Pi", "NexusIM", $model);
                } else {
                    return 'Unknown Device';
                }
            }
